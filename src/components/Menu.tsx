...
export default function Menu(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function getBranches() {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}v1/shop/getbranches`).then(async (res) => {
        let data = await res.json();
        if (isSubscribe) {
          setBranches(data);
          if (
            data.length === 1 &&
            (localStorage.getItem('branch') === undefined || localStorage.getItem('branch') === null || localStorage.getItem('branch') === '')
          ) {
            ...
            {isAuth ? (
              <>
                ...
                <div id={styles['account-menu']}>
                  <ul>
                    <li>
                      <Link href={String(process.env.NEXT_PUBLIC_MEMBER_DASHBOARD_URL)}>Profile</Link>
                    </li>
          ...
          <div className={styles.navMenuCart}>
            ...
            <label htmlFor={styles['cart-button']}>
              <div className={styles.cartBtn}>
                <div
                  id={styles['cart-count']}
                  className="rounded-full bg-red-600 text-xs absolute w-5 h-5 flex items-center justify-center top-0 right-0 text-white"
                >
                  {cartItems.length}
                </div>
            ...
            <div className={styles.cartList}>
              <div className="text-gray-600 text-3xl font-bold border-b-4 mb-3 border-pizza-600">Your cart</div>
              {branches && <CartBranchSelection branches={branches} />}
              {cartItems.length > 0 ? (
                cartItems.map((item: Cart) => <CartCard cartInfo={item} key={item.id} />)
              ) : (
                <>
