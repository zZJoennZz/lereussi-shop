        ...
        <div className="lws-container-inner">
          ...
          <div className={styles.productPageHeader}>
            ...
            <div className="flex items-center text-sm justify-end col-span-5">
              <label htmlFor="sort-by" className="text-slate-500">
                Sort by
              </label>
          ...
          <div>
                  ...
                  <div key={item.variant_id} className="col-span-12 md:col-span-6 xl:col-span-4 p-1">
                    <div className={styles.productsListCard}>
                      ...
                      <div className="w-full grid grid-cols-2 pt-2 space-x-1">
                        {item.stocks === 0 ? (
                          <>
                            <div className="bg-red-600 text-white text-center col-span-2 text-sm py-1 rounded">Availability: Out of stock!</div>
                          </>
                        ) : (
                          <>
                            <div>
                              <button onClick={() => addToCart(item.variant_id)} className={styles.addToCartBtn}>
                                <ShoppingCartIcon className="inline h-3 w-3 mr-1" />
                                Add to cart
                              </button>
                            </div>
                            <div>
                              <Link href={`/product/${item.meta.page_slug}`} className={styles.viewDetailBtn}>
                                <MagnifyingGlassIcon className="inline h-3 w-3 mr-1" />
                                View Detail
                              </Link>
                            </div>
                          </>
            ...
            <Pagination items={products.length} currentPage={currentPage} pageSize={pageSize} onPageChange={onPageChange} />
          </div>
        </div>
      </div>
    </>
