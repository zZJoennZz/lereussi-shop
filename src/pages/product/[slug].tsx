...
export default function ProductPage({ product }: { product: ProductVariant }): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function getStock() {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}v1/shop/getproductvariant/?` +
          new URLSearchParams({
            slug: product.meta.page_slug,
            ...
            <div className="col-span-12 md:col-span-5 order-1 md:order-2 mb-2 md:mb-0">
              <div ref={sliderRef} className="keen-slider min-h-[250px] mb-2">
                <div
                  ...
                  style={{ backgroundImage: `url(${product.variant_image})`, backgroundPosition: 'center', backgroundSize: 'cover' }}
                ></div>
                {product.media.map((media) => (
                  <div
                    key={media.id}
                    className="keen-slider__slide"
                    style={{ backgroundImage: `url(${media.attachment})`, backgroundPosition: 'center', backgroundSize: 'cover' }}
                  ></div>
                ))}
              </div>
